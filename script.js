...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Upload music button
    uploadMusicBtn.addEventListener('click', async function() {
        ...
        // If Electron is available, we can use the native file dialog
        if (window.electron) {
            try {
                ...
                uploadStatus.textContent = 'Error opening file dialog.';
            }
        }
    });
    
    // File input change handler (fallback for browser)
    ...
    // Process files selected via Electron dialog
    async function processElectronFiles(files) {
    ...
    // Process audio files (browser method)
